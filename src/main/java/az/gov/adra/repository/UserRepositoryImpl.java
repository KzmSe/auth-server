...
public class UserRepositoryImpl implements UserRepository {
    ...
    private static final String FIND_USERS_BY_BIRTH_DATE_SQL = "SELECT u.name, u.surname, u.username, u.img_url, date_of_birth, p.name as position_name FROM  users u inner join Position p on u.position_id = p.id WHERE DATEADD (YEAR, DATEPART(YEAR, GETDATE()) - DATEPART(YEAR, u.date_of_birth), u.date_of_birth) BETWEEN CAST(GETDATE() AS DATE) AND CAST(DATEADD(WEEK, 4, GETDATE())-1 AS DATE) AND u.enabled = ? ORDER BY MONTH(u.date_of_birth), DAY(u.date_of_birth) OFFSET ? ROWS FETCH NEXT ? ROWS ONLY";
    private static final String FIND_TOP_USERS_BY_BIRTH_DATE_SQL = "SELECT TOP 3 u.name, u.surname, u.username, u.img_url, date_of_birth, p.name as position_name FROM  users u inner join Position p on u.position_id = p.id WHERE DATEADD (YEAR, DATEPART(YEAR, GETDATE()) - DATEPART(YEAR, u.date_of_birth), u.date_of_birth) BETWEEN CAST(GETDATE() AS DATE) AND CAST(DATEADD(WEEK, 4, GETDATE())-1 AS DATE) AND u.enabled = ? ORDER BY MONTH(u.date_of_birth), DAY(u.date_of_birth)";
